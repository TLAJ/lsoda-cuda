...
__device__ int dlsoda_(Fex f, int *neq, double *y, double *t, double *tout, int *itol, double *rtol, double *atol, int *itask, int *istate, int *iopt, double *rwork, int *lrw, int *iwork, int *liw, Jex jac, int *jt, struct cuLsodaCommonBlock *common, double *param)
{
    /* Initialized data */
...
L110:
    ...
    lf0 = lyh + nyh;
    f(neq, t, y, &rwork[lf0 -1], param); //fixed neq y
    nfe = 1;
...
L290:
	...
	/* ----------------------------------------------------------------------- */
    dstoda_(neq, y, &rwork[lyh -1], &nyh, &rwork[lyh -1], &rwork[lewt -1], &rwork[lsavf -1], &rwork[lacor -1], &rwork[lwm -1], &iwork[liwm -1], f, jac, common, param); //fixed neq y
    kgo = 1 - kflag;
    switch (kgo) {
		case 1:  goto L300;
		case 2:  goto L530;
...
#endif
template<typename Fex, typename Jex> 
__device__ int dstoda_(int *neq, double *y, double *yh, int *NOT_nyh, double *yh1, double *ewt, double *savf, double *acor, double *wm, int *iwm, Fex f, Jex jac, struct cuLsodaCommonBlock *common, double *param)
{
    /* Initialized data */
...
L220:
    ...
    f(neq, &tn, y, savf, param); //fixed neq y
    ++nfe;
    if (ipup <= 0) {
		goto L250;
    }
	...
	/* ----------------------------------------------------------------------- */
    dprja_(neq, y, &yh[yh_offset -yh_offset], NOT_nyh, ewt, acor, savf, wm, iwm, f, jac, common, param);  //fixed neq y
    ipup = 0;
    rc = 1.;
    nslp = nst;
    crate = .7;
...
L640:
    ...
    f(neq, &tn, y, savf, param); //fixed neq
    ++nfe;
    i__1 = n;
    for (i__ = 1; i__ <= i__1; ++i__) {
		/* L650: */
...
#endif 
template<typename Fex, typename Jex> 
__device__ int dprja_(int *neq, double *y, double *yh, int *NOT_nyh, double *ewt, double *ftem, double *savf, double *wm, int *iwm, Fex f, Jex jac, struct cuLsodaCommonBlock *common, double *param)
{
    /* System generated locals */
...
L200:
    ...
    for (j = 1; j <= i__1; ++j) {
		...
		y[j - 1] += r__;
		fac = -hl0 / r__;
		f(neq, &tn, y, ftem, param); //fixed neq
		i__2 = n;
		for (i__ = 1; i__ <= i__2; ++i__) {
			/* L220: */
